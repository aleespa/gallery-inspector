...
def main():
    ...
    for target in photo_targets:
        filter_files(files=files, output_dir=target, options=options, query=photo_query)

    # Process and organize videos to all video target directories
    for target in video_targets:
        filter_files(files=files, output_dir=target, options=options, query=video_query)
