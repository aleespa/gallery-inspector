...
def plot_sunburst(df: pd.DataFrame, path_column: str = 'directory', size_column: str = 'size (MB)'):
    """
    ...
    try:
        ...
        df['rel_path'] = df[path_column].apply(lambda x: os.path.relpath(x, common_prefix))
    except Exception:
...
def plot_size_distribution(df: pd.DataFrame):
    ...
    """
    fig = px.histogram(df, x='size (MB)', nbins=50,
