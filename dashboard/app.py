...
def load_all_data(file_path):
    ...
    def _load(name):
        return (
            pd.read_excel(xls, sheet_name=name)
    ...
    for df in (df_images, df_videos):
        if "date_taken" in df.columns:
            ...
            df["date_taken"] = pd.to_datetime(
                df["date_taken"], errors="coerce", format="mixed"
            )
...
def main_app_block(df_images, df_videos, df_others):
    ...
    total_size = sum(
        df["size (MB)"].sum() for df in (fi, fv, df_others) if "size (MB)" in df.columns
    )
    ...
    h, m, s = (
        int(total_duration_sec // 3600),
        int((total_duration_sec % 3600) // 60),
        int(total_duration_sec % 60),
    )
...
def _stacked_timeline(df: pd.DataFrame, group_by: str, time_by: str, title: str):
    if "date_taken" not in df.columns or group_by not in df.columns or df.empty:
        return
    st.header("Photography Timeline")
    tmp = df.copy()
    tmp["period"] = (
        tmp["date_taken"].dt.year.astype(str)
        if time_by == "year"
        else tmp["date_taken"].dt.to_period("M").astype(str)
    )
    ...
    fig = px.bar(
        ...
        color=group_by,
        barmode="stack",
        color_discrete_sequence=STACKED_COLORS,
        title=title,
    )
...
def _video_recording_summary(fv: pd.DataFrame):
    ...
    tmp_v["date_taken"] = pd.to_datetime(
        tmp_v["date_taken"], errors="coerce", format="mixed"
    )
    ...
    fig = px.bar(
        ...
        x="month",
        y="minutes",
        title="Minutes Recorded per Month",
    ...
    with dist_col1:
        ...
        if counts:
            fig = px.pie(
                ...
                color_discrete_sequence=["#66b3ff", "#99ff99", "#ff9999"],
                hole=0.4,
            )
...
def _camera_settings_distribution(fi: pd.DataFrame):
    ...
    if not fi.empty:
        settings_available = [
            c
            for c in ("aperture", "shutter_speed", "iso", "focal_length")
            if c in fi.columns
        ]
        if settings_available:
            fig = make_subplots(
                rows=2,
                cols=2,
                subplot_titles=["Aperture", "Shutter Speed", "ISO", "Focal Length"],
            )
            ...
            def _log_hist(fig, series, row, col, color, tickvals, ticktext, name):
                ...
                valid = [
                    ...
                    if log_data.min() * 0.9 <= np.log10(v) <= log_data.max() * 1.1
                ]
                if valid:
                    v_t, t_t = zip(*valid)
                    fig.update_xaxes(
                        tickvals=[np.log10(v) for v in v_t],
                        ticktext=list(t_t),
                        row=row,
                        col=col,
                        tickangle=45,
            ...
            _log_hist(
                fig,
                fi.get("aperture", pd.Series()),
                1,
                1,
                "coral",
                STD_APERTURES,
                [f"f{v:g}" for v in STD_APERTURES],
                "Aperture",
            )
            ss = (
                fi["shutter_speed"].apply(_parse_shutter)
                if "shutter_speed" in fi.columns
                else pd.Series()
            )
            _log_hist(
                fig,
                ...
                "orchid",
                STD_SHUTTERS,
                [f"1/{int(round(1/v))}" if v < 1 else f"{v:g}s" for v in STD_SHUTTERS],
                "Shutter",
            )
            _log_hist(
                fig,
                fi.get("iso", pd.Series()),
                2,
                1,
                "gold",
                STD_ISOS,
                [str(v) for v in STD_ISOS],
                "ISO",
            )
            _log_hist(
                fig,
                fi.get("focal_length", pd.Series()),
                2,
                2,
                "teal",
                STD_FOCALS,
                [f"{v}mm" for v in STD_FOCALS],
                "Focal Length",
            )
...
def _photo_map(fi: pd.DataFrame):
    has_coords = (
        not fi.empty
        and "latitude" in fi.columns
        and "longitude" in fi.columns
        and fi[["latitude", "longitude"]].dropna().shape[0] > 0
    )
    if not has_coords:
        return
    st.header("Photo Map")
    ...
    fig_map.update_layout(
        height=600, margin=dict(l=0, r=0, t=30, b=0), coloraxis_showscale=False
    )
...
def _data_tables(fi: pd.DataFrame, fv: pd.DataFrame, df_others: pd.DataFrame):
    ...
    t1, t2, t3 = st.tabs(
        [
            f"📷 Images ({len(fi):,})",
            f"🎬 Videos ({len(fv):,})",
            f"📄 Others ({len(df_others):,})",
        ]
    )
    with t1:
        st.dataframe(fi, width="stretch", height=300)
    with t2:
        st.dataframe(fv, width="stretch", height=300)
    with t3:
        st.dataframe(df_others, width="stretch", height=300)
